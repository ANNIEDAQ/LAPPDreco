...
//Set values for the Waveform members
  ...
  */
  ...
  //  s->TSpectrum::Background(&(bg[0]),DimSize,8,TSpectrum::kBackDecreasingWindow,TSpectrum::kBackOrder2,kTRUE, TSpectrum::kBackSmoothing3,kFALSE);
  //  s->TSpectrum::Background(&(bg[0]),DimSize,4,TSpectrum::kBackIncreasingWindow,TSpectrum::kBackOrder2,kTRUE, TSpectrum::kBackSmoothing3,kFALSE);

  //  TF1* sinit0 = new TF1("sinit0","([0]*sin(.00066*x+[1]))+[2]",0,DimSize*Deltat);
  //  TF1* sinit1 = new TF1("sinit1","([0]*sin(.00055*x+[1]))+[2]",0,DimSize*Deltat);
  TF1* sinit0 = new TF1("sinit0","([0]*sin([3]*x+[1]))+[2]",0,DimSize*Deltat); //0.000557
  TF1* sinit1 = new TF1("sinit1","([0]*sin([3]*x+[1]))+[2]",0,DimSize*Deltat); //0.000556
  ...
  bool fitsinewave=false; //CONTROLS WHETHER OR NOT THE SINE WAVE IS TAKEN OUT
  ...
  if(fitsinewave==true)
    {
      if(nottrig==0) hwav_raw->Fit("sinit0","q","",32000,50000);
  ...
  if (fitsinewave==true)
	...
	if(nottrig==0) hwav->SetBinContent(i+1,vol[i]-(sinit0->Eval(hwav->GetBinCenter(i+1))));
	if(nottrig==1) hwav->SetBinContent(i+1,vol[i]-(sinit1->Eval(hwav->GetBinCenter(i+1))));
	if(nottrig==2) hwav->SetBinContent(i+1,vol[i]-(sinit1->Eval(hwav->GetBinCenter(i+1))));
	if(nottrig<0) hwav->SetBinContent(i+1,vol[i]);
	//hwav->SetBinError(i+1,0.3);
	//hbg->SetBinContent(i+1,-bg[i]);
	if(clipit){
	  if((hwav->GetBinCenter(i+1))<cliplow) hwav->SetBinContent(i+1,0.0);
	  if((hwav->GetBinCenter(i+1))>cliphi) hwav->SetBinContent(i+1,0.0);
	}
      }
    }
  else
    {
      for(int i=0;i<DimSize;i++) {
	if(nottrig==0) hwav->SetBinContent(i+1,vol[i]);
	if(nottrig==1) hwav->SetBinContent(i+1,vol[i]);
	if(nottrig==2) hwav->SetBinContent(i+1,vol[i]);
	if(nottrig<0) hwav->SetBinContent(i+1,vol[i]);
      }
    }
  AmpThreshold = 60;
