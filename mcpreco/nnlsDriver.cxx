...
int main (int argc, char** argv) 
{
  ...
  TH1D* xout = new TH1D("x vector","x vector",512,0,512);
  TH1D* input = new TH1D("input","input",512,0,512);
  TH1D* output = new TH1D("output","output",512,0,512);
  double outputa[512];
  for(int i=0; i<x->length(); i++) outputa[i]=0;
  double chi2 = 0;
  TString name = "A";
  ...
  for(int i=0; i<x->length(); i++)
    {
      name+=i;
      TH1D* matri = new TH1D(name,name,512,0,512);
      //error(1,stderr,"%f\n",(x->getData())[i]);
      xout->SetBinContent(i+1,x->getData()[i]);
      input->SetBinContent(i+1,b->getData()[i]);
      
      for(int j=0; j<A->ncols(); j++)
	{
	  outputa[j]+=(x->getData()[i])*(A->get(i,j));
	  matri->Fill(j,A->get(i,j));
	}
      name="A";
      if(i<30 || i>482) matri->Write();
      delete matri;
    }
  ...
  for(int i=0; i<x->length(); i++)
    {
      if(x->getData()[i]!=0) chi2+=(outputa[i]-(x->getData()[i]))*(outputa[i]-(x->getData()[i]))/(x->getData()[i]);
    }

  for(int i=0; i<x->length(); i++)
    {
      output->SetBinContent(i+1,outputa[i]);
    }    
  f->cd();
  xout->Write();
  input->Write();
  output->Write();
  TCanvas* c1 = new TCanvas("comparison","comparison",600,600);
